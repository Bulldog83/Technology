...
global chiselBlockGroups as IItemStack[][string] = {
	"sandstoneyellow" : [
		<quark:sandstone_new>
	],
	"sandstonered" : [
...
val removeBlacklist as IItemStack[] = [
	<immersiveengineering:storage:6>,
	<thermalfoundation:storage:7>,
	<thermalfoundation:storage_alloy:4>,
	<thaumcraft:metal_brass>,
...
for group in chiselGroupsNew {
	mods.chisel.Carving.addGroup(group);
}

for group, blocks in chiselBlockGroups {
	for block in blocks {
		mods.chisel.Carving.addVariation(group, block);
		if(!(removeBlacklist in block)) {
			Disabling.markForRemove(block);
		}
	}
}
